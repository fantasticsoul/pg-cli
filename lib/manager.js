...
function generateSqlItem(operation, cmd) {
  ...
  switch (cmd) {
    case '$insert':
      ...
      break;
    case '$insertBatch':
      if(payload.toInsertBatch.length==0)throw new Error('queryWithTransaction: $insertBatch can not accept empty array!');
      sc = sqlComposer.prepareBatchInsertSql(payload.tableName, payload.toInsertBatch, true);
      break;
    case '$update':
      sc = sqlComposer.prepareUpdateSql(payload.table, payload.filter, payload.toUpdate, true);
      break;
    case '$updateBatch':
      if(payload.toUpdateBatch.length==0)throw new Error('queryWithTransaction: $updateBatch can not accept empty array!');
      sc = sqlComposer.prepareBatchUpdateSql(payload.tableName, payload.toUpdateBatch, payload.filterKey, payload.returnFields);
      break;
    case '$select':
 ...
 * {'$updateBatch':{table:'company',toUpdateBatch:{...},filterKey:'id',returnFields:['id','name'],key:'insertCompany'}}
