    ...
    if (err) {
      ...
      client.query(sql, args, function (err, result) {
        ...
        if (err) {
          ...
          //console.log(sql,args,result.rows);
  ...
  try{
    ...
    switch (cmd) {
      case '$insert':
        ...
        break;
      case '$insertBatch':
        if(payload.toInserts.length==0)throw new Error('[generateSqlItem] $insertBatch can not accept empty array!');
        sc = sqlComposer.prepareBatchInsertSql(payload.table, payload.toInserts, true);
        break;
      case '$update':
...
function insertBatch(tableName, toInserts, cb) {
  try{
    var sc = sqlComposer.prepareBatchInsertSql(tableName, toInserts, true);
    query(sc.sql, sc.args, cb);
  }catch(ex){
    cb(ex.message);
  }
